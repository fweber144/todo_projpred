...
* From time to time: Search for `do.call` to check whether we might have costly calls of it (i.e., `do.call()` calls with arguments that consist of large objects, e.g., `stanreg`s or `brmsfit`s). In such cases, we should use direct functions calls where possible and otherwise `projpred:::do_call()`. This should speed up the corresponding code. Note that `do_call()` seems to provide a speed-up in case of a few large arguments, but should not be used in case of a large number of arguments (see `?brms::do_call`).
* Each time a new **lme4** version is out on CRAN, we need to diff **lme4** functions `mkNewReTrms()` and related functions (`predict.merMod()`, for example) between the former **lme4** version and the updated one. If there are changes, we might have to update `repair_re()` correspondingly.
    ...
    &rarr; Last check (November 6, 2023): Everything OK for **lme4** v1.1-35.1.
...
* In `plot.vsel()`, try to put the submodel sizes just above the x-axis or on a secondary x-axis shown at the top of the plot (the predictor names, however, should still be shown at the bottom, i.e., on the primary x-axis). This might make it possible to use `text_angle = 90` in case of small plot dimensions (where `text_angle = 90` would lead to overplotting of the x-axis tick labels). In such cases, `text_angle = NULL` and `ranking_abbreviate = TRUE` might help as well; the abbreviation could be further refined via argument `ranking_abbreviate_args` (and a table of the abbreviations could be created via output attribute `projpred_ranking_abbreviated`). Another possibility how `text_angle = 90` could work better might be to remove the `\n` newline in the x-axis tick labels, put the submodel size after the name of the predictor term (separated, e.g., by `:` if that does not lead to confusion with interaction terms, otherwise put the submodel size in parentheses), and then right-align.
...
* Currently, `brms:::get_refmodel.brmsfit()` ignores `wrhs` and `orhs` in the `extract_model_data` function (more precisely, in `brms:::.extract_model_data()`). This is probably OK because this makes `brms:::.extract_model_data()` follow the **brms** style of handling observation weights and offsets for predictions and also because it is documented in `brms:::get_refmodel.brmsfit()`, but for consistency with **rstanarm**, it might be better to not ignore `wrhs` and `orhs`. When fixing this, don't forget about the binomial family with > 1 trial per observation (for example:
    ...
    pln_fml <- proj_linpred(prj,
                            ...
                            allow_nonconst_wdraws_prj = TRUE)
    ```
* Fix subsampled PSIS-LOO CV, see issue [#94](https://github.com/stan-dev/projpred/issues/94). Note that there is [this comment](https://github.com/stan-dev/projpred/blob/7e67eed52cd307954f47cea939a6a595b07e1bfb/R/cv_varsel.R#L789-L793) which gives a hint that **projpred** might want to use the approach by Magnusson et al. (2019). However, for this approach, we might have to implement the following (note that the following list might not be exhaustive and might be even incorrect, as it was written some time ago):
    1. In `loo_subsample_pps()`: definition of `wcv` via the logarithms of the predictive densities (not via the predictive densities themselves);
    1. In `loo_subsample_pps()`: sampling *with* replacement;
    1. In aggregations across the observations: `1 / wcv` instead of `wcv`;
    1. In aggregations across the observations: usage of the special variance estimator;
    1. **projpred** probably has to use the HH estimator every time it aggregates over the subsampled PSIS-LOO CV folds (i.e., also in `cv_proportions.ranking()`, for example). And where the SE of such an aggregated value is calculated, this also needs to be done by accounting for the HH estimator.
